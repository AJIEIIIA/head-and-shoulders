...
function CandleStickChart(width, height, data, options){
...
CandleStickChart.prototype.prepareLayout = function(){
     ...
     var max = this.data[lastIndex].high;
     var min = this.data[lastIndex].low;
     
    
     //minimal step by time between elements
     var minTimeStep = null;
     //will find aproximate minimal step
     //var minMove = Math.abs(this.data[lastIndex].open - this.data[lastIndex].close);
     var maxDecimals = 2;
     for (var i = 1; i < maxElement2Display; i++){
          var index = lastIndex - i;
          if (index < 0) break;
          
          var item = this.data[index];
          var move = Math.abs(item.open - item.close);
          
	  /*
          var decimals = helpers.getDecimalPlaces(item.close);
          if (maxDecimals < decimals)
               maxDecimals = decimals;
	  */
          //if (minMove == 0) minMove = step;
          //if (move && move < minMove) minMove = move;
     ...
     min = min - min % yStepSize;
     max = max + yStepSize - max % yStepSize;
	...
	return;
     ...
     //this.view.grid_ctx.closePath();
    
     //this.view.main_ctx.translate(.5, 0.5);
     //this.view.grid_ctx.translate(.5, 0.5);
     //this.view.xAxis_ctx.translate(0.5, 0.5);
     //this.view.yAxis_ctx.translate(0.5, 0.5);
...
CandleStickChart.prototype.prepareLayout = function() {
    ...
    while (totalSteps > maxYSteps || totalSteps * 2 < maxYSteps) {
        if (totalSteps > maxYSteps) {
            ...
            var currentStepSize = yStepSize / 2;
            //we need to asure that step size is not to low compared to minMove
            if (helpers.getDecimalPlaces(currentStepSize) <= maxDecimals) {
                yStepSize = currentStepSize;
                totalSteps = Math.round(diff / yStepSize);
            } else break; //if it's too low - end cycle
        }
...
CandleStickChart.prototype.setupLayout = function() {
    //layout container
    this.view.container.style.width = helpers.stylePx(this.width);
    this.view.container.style.height = helpers.stylePx(this.height);
...
CandleStickChart.prototype.draw = function () {
    ...
    var ordered = this.data.slice(0).sort(function(c1, c2) {
        if (c1.isNegative() && c2.isPositive())
            return 1;
        else return -1;
    });
    ...
    function drawCandle(candle, mid, width, padding, yfunc, context) {
        ...
        var left = mid - width / 2 + padding;
        var trueWidth = xStep - 2 * padding;
        ...
        if (candle.isNeutral()) {
            context.moveTo(helpers.normalizeX(left), yfunc(maj));
            context.lineTo(helpers.normalizeX(left + trueWidth), yfunc(maj));
        }
        else {
            context.rect(helpers.normalizeX(left), yfunc(maj), trueWidth, yfunc(min) - yfunc(maj));
            //context.fill();
        }
    };

    //prepare
    var candleSpacing = Math.round(0.2 * xStep);
    ...
    var boldFont = "bold " + helpers.makeFont(this.options.axisFontSize, this.options.axisFont);
    var font = helpers.makeFont(this.options.axisFontSize, this.options.axisFont);
    this.view.xAxis_ctx.font = font;
    ...
    for (var i = this.lastIndex; i >= 0; i--) {
        if (i < this.data.length) {
            var item = this.data[i];
            var left = x - xStep / 2 + candleSpacing;
            if (item.isNeutral() || item.isNegative()) { //draw full candle		
                drawCandle.call(this, item, x, xStep, candleSpacing, calculateY, this.view.main_ctx);
            }
            //display x axis mark if necessary
            var displayXMark = false;
            var xMark = "";
            ...
            if (displayXMark) {
                ...
                if (boldMark) {
                    if (!currentBold) {
                        //make font bold
                        this.view.xAxis_ctx.font = boldFont;
                    }
                    this.view.xAxis_ctx.fillText(xMark, markX, AXIS_MARK_SIZE);
                } else {
                    if (currentBold) {
